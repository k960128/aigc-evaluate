package com.chinatelecom.aigc.evaluate.job.core.handler;
...
public class JobHandlerInvoker extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext executionContext) throws JobExecutionException {
        ...
        Integer maxThreadSize = executionContext.getMergedJobDataMap().getInt(JobDataKeyEnum.JOB_MAX_THREAD_SIZE.name());
        ...
        log.info("taskId:{} jobHandlerName:{} jobHandlerParam:{}", jobId, jobHandlerName, jobHandlerParam);
        // 解析 JSON
        Map<String, Object> paramMap = JSONObject.parseObject(jobHandlerParam, HashMap.class);
        paramMap.put("taskId", jobId);
        paramMap.put("maxThreadSize", maxThreadSize);

        // 更新参数
        jobHandlerParam = JSONObject.toJSONString(paramMap);
        ...
        try {
            // 记录 Job 日志（初始）
            //jobLogId = jobLogFrameworkService.createJobLog(jobId, startTime, jobHandlerName, jobHandlerParam, jobModelId, jobDatasetIds, refireCount + 1);
