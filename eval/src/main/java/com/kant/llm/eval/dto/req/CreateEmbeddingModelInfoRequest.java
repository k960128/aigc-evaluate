 ...
 * 创建向量模型信息请求参数。
 *
 * <p>该对象用于接收前端或平台内部管理端在新增 Embedding 模型时提交的模型基础信息、
 * 调用连接信息以及向量能力参数。创建成功后，这些字段会被持久化到
 * embedding_model_info 表中。</p>
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class CreateEmbeddingModelInfoRequest {

    /** 向量模型名称，通常为供应商侧真实模型名称，例如 text-embedding-3-large、bge-m3。 */
    private String model;

    /** 平台内部模型编码，用于业务系统稳定引用该模型，建议在平台内保持唯一。 */
    private String modelCode;

    /** 模型厂商编码，对应模型厂商表中的 manufacturer_code，用于区分 OpenAI、通义千问、智谱等供应商。 */
    ...
    /** 模型输出的向量维度，例如 768、1024、1536、3072，后续创建向量库集合时需要保持一致。 */
